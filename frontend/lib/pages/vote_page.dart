...
class _VotePageState extends State<VotePage> {
  ...
  void castUserVote(VoteOrientation votingOrientation) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Error casting your vote')));
      setState(() {
        _isLoading = false;
      });
    }
  ...
  void getRandomProposal() async {
    final session = context.read<AuthController>().session;
    final proposalCriteria = session?.proposalCriteria;
    if (proposalCriteria == null) {
      setState(() {
        _isLoading = false;
      });
      return;
    ...
    try {
      Proposal? proposal = await voteController.getRandomProposal(
        proposalCriteria,
      );
      if (!mounted) return;
      setState(() {
